# Code QA Task — Code Onboarding & Comprehension (Bash / Bitcoin Regtest)

## prompt_title

What Is This Bitcoin Miner Script Actually Doing?

## prompt_statement

I just joined the Stacks hacknet infrastructure team and this Bitcoin miner script is the first thing I've been asked to understand before I touch anything. It runs inside the Bitcoin regtest Docker container and is responsible for setting up wallets and keeping the local development chain moving. I've worked with bash scripts before but I've never dealt with Bitcoin regtest or the Stacks mining layer, and there are several things in this file I genuinely can't follow.

At the very top, two `trap` lines are set up — one traps `INT` and `TERM` to call `exit`, and the other traps `EXIT` to call `kill 0`. I understand that `trap` catches signals, but I don't know why `kill 0` is used specifically on the `EXIT` trap rather than just letting the script exit naturally. What does sending signal zero to process group zero accomplish, and why is this pattern needed in a script that runs background `sleep` commands?

The script uses `compgen -A variable | grep "STACKS.*.BTC_ADDR"` to discover miner addresses, then `mapfile -t ADDRESSES` to load them into an array. I've used `grep` many times but I've never seen `compgen -A variable` in a production script — I don't understand why the script discovers environment variables by name pattern at runtime rather than accepting a fixed list of addresses. What is this pattern doing and why would you design it this way?

The `init` function handles genesis block mining, and there's a `RESERVED_BLOCKS=100` constant with a comment explaining that the 100 blocks are reserved so earlier blocks have received rewards by the epoch 2.0 block. I don't understand what that reservation is accomplishing — why is the script mining initial blocks to stacks-miner wallet addresses but then mining the final 100 blocks to a separate depositor wallet, and why does the number 100 specifically matter for block maturity?

Inside `mining_loop`, the script loops through all miner wallets, calls `listtransactions` on each one, and sums up a `conf_counter`. If that counter is zero or a 30-second timeout has been reached, it mines a Bitcoin block. I can't figure out what it's detecting — what is a "Stacks mining mempool tx" in a Bitcoin regtest context, and why does the presence or absence of that kind of transaction drive whether a Bitcoin block gets produced?

Also in `mining_loop`, blocks are mined to `bitcoin_block_producer_addr` — a dedicated address — instead of to any of the stacks-miner addresses that were used during `init`. Why does the script switch to a separate block producer address during continuous mining rather than just continuing to mine to the same addresses?

Finally, `DEFAULT_TIMEOUT` is initialized to current epoch plus 30 seconds at the top of the file, but it gets reset to a fresh `$(date +%s) + 30` inside the loop every time a block is mined. Why is this a rolling 30-second window that resets on each block rather than a fixed `sleep 30` at the bottom of the loop?
